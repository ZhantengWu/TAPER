"""
Baseline B: ZXing decoding without any preprocessing.
This provides an alternative baseline using ZXing library for comparison.
"""
...
def run_baseline_b(img_dir: str, output_json: str, time_budget_ms: float = None, verbose: bool = False) -> None:
    """
    Run baseline B (direct ZXing decoding) on a directory of images.
    ...
    # Initialize ZXing decoder backend
    try:
        backend = create_backend("zxing")
        print(f"Using decoder backend: {backend.name}")
    except Exception as e:
        print(f"Error initializing ZXing backend: {e}")
        print("Make sure ZXing jar file is available")
        return
    ...
    for i, img_path in enumerate(image_files):
        ...
        # Direct ZXing decoding (baseline B)
...
def main():
    parser = argparse.ArgumentParser(description="Baseline B: Direct ZXing decoding")
    ...
    try:
        run_baseline_b(args.img_dir, args.out, args.time_budget_ms, args.verbose)
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
