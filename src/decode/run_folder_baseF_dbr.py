"""
Baseline F: Direct Dynamsoft Barcode Reader decoding without any preprocessing.
Commercial-grade barcode recognition engine for comparison.
"""
...
from decode.backend import DBRBackend


def run_baseline_f(img_dir: str, output_json: str, time_budget_ms: int = None, verbose: bool = False) -> None:
    """
    Run baseline F (direct DBR decoding) on a directory of images.
    ...
    backend = DBRBackend()
    if backend.reader is None:
        print("Error: DBR not available. Please install dbr package.")
        return
    ...
    for i, img_path in enumerate(image_files):
        ...
        # Direct DBR decoding (baseline F)
        texts = backend.decode(img)
        elapsed_ms = timer.stop() * 1000.0
        
        # Check timeout after decoding
        is_timeout = time_budget_ms is not None and elapsed_ms >= time_budget_ms
        if is_timeout:
            stage = "timeout"
        else:
            stage = "direct"
...
def main():
    parser = argparse.ArgumentParser(description="Baseline F: Direct DBR decoding")
    parser.add_argument("--img_dir", required=True, help="Directory containing images")
    parser.add_argument("--out", required=True, help="Output JSON file path")
    parser.add_argument("--time_budget_ms", type=int, help="Time budget per image in milliseconds")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    
    args = parser.parse_args()
    
    try:
        run_baseline_f(args.img_dir, args.out, args.time_budget_ms, args.verbose)
