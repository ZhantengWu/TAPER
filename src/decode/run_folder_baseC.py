...
This baseline applies upsampling before decoding to improve small target detection.
"""
...
def run_baseline_c(img_dir: str, output_json: str, scale: float = 1.5, time_budget_ms: float = None, verbose: bool = False) -> None:
    """
    Run baseline C (upsampling + pyzbar decoding) on a directory of images.
    ...
    print(f"Upsampling scale: {scale}x")
    ...
    for i, img_path in enumerate(image_files):
        ...
        # Upsample then decode (baseline C)
        img_upsampled = upsample(img, scale)
        texts = backend.decode(img_upsampled)
...
def main():
    parser = argparse.ArgumentParser(description="Baseline C: Upsampling + pyzbar decoding")
    parser.add_argument("--img_dir", required=True, help="Directory containing images")
    parser.add_argument("--out", required=True, help="Output JSON file path")
    parser.add_argument("--scale", type=float, default=1.5, help="Upsampling scale factor")
    ...
    try:
        run_baseline_c(args.img_dir, args.out, args.scale, args.time_budget_ms, args.verbose)
